...
import { DashboardIcon, ElectionIcon, CandidatesIcon, ResultsIcon, MessagesIcon, SettingsIcon, LogoutIcon, ShieldCheckIcon, UserGroupIcon } from '../../components/icons';

import AdminDashboardPage from './AdminDashboardPage';
import ElectionManagementPage from './ElectionManagementPage';
...
const AdminSidebar = () => {
    ...
    const navItems = [
        ...
        { name: 'Elections', path: '/admin/elections', icon: ElectionIcon },
        { name: 'Candidates', path: '/admin/candidates', icon: CandidatesIcon },
        { name: 'Users', path: '/admin/users', icon: UserGroupIcon },
        { name: 'Results', path: '/admin/results', icon: ResultsIcon },
        { name: 'Messages', path: '/admin/messages', icon: MessagesIcon },
    ...
    return (
         <aside className="w-72 bg-gradient-to-b from-slate-900 to-slate-800 text-gray-200 flex flex-col min-h-screen shadow-2xl z-10">
            <div className="h-20 flex items-center justify-center border-b border-slate-700/50 bg-slate-900/50 backdrop-blur-sm">
                 ...
                 <span className="ml-3 text-2xl font-bold tracking-tight text-white">Admin Portal</span>
            </div>
                 ...
                 <div className="flex items-center mb-6 p-3 bg-slate-800/80 rounded-xl border border-slate-700/50 shadow-sm">
                    {auth?.photo && auth.photo !== 'no-photo.jpg' ? (
                        <img 
                            ...
                            alt={auth.user} 
                            className="w-10 h-10 rounded-full object-cover border-2 border-brand-blue shadow-md"
                        />
                    ) : (
                        <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-brand-blue to-purple-600 flex items-center justify-center font-bold text-white uppercase shadow-md">
                            {auth?.user?.charAt(0)}
                        </div>
                    )}
                    <div className="ml-3 overflow-hidden">
                        <p className="font-semibold text-sm capitalize text-white truncate">{auth?.user}</p>
        ...
        <div className="flex bg-gray-50 font-sans">
            ...
            <main className="flex-1 p-8 lg:p-12 h-screen overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
                <Routes>
                    ...
                    <Route path="messages" element={<UserComplaintsPage />} />
